      ...
      if (res.data.success) {
        ...
        if (res.data.matches.pmatch) {
          ...
          if (res.data.matches.pmatch._id != gameid) {
            navigate("/speed-ludo-game/" + res.data.matches.pmatch._id);
          }
        }
  ...
  return (
    <>
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}>
        <SpeedLudoHead title="Speed Ludo Lite" />
      </motion.div>
      {pmatch && <PMatchSpeed match={pmatch} />}

      {omatches &&
        omatches.length > 0 &&
        omatches.map((match, index) => (
          <SpeedLudoItem key={match.key} data={match} lite={true} />
        ))}
    </>
  );
};
